...
import { mapUserToUserResponse, userLoginResponse } from '@/modules/users/mappers/userResponseMapper';
...
describe("Get users", ()=> {
    ...
    it("should return users json body", async()=> {
        (
            fetchUser as jest.MockedFunction<typeof fetchUser>
    ...
    it("should return bad request when users is null", async()=> {
        ...
        await expect(()=> UserService.getUsers()).rejects.toThrow(AppError.badRequest(`Error while retrieving users.`))
    });
});
...
describe("User signup", ()=> {
    ...
    it("should be able to sigh up", async()=> {
        ...
        (
            hash as jest.MockedFunction<typeof hash>
        ).mockImplementation(()=> Promise.resolve(userCreateDto.password));
    ...
    it("should return bad request when existing user is found", async()=> {
        ...
        await expect(()=> UserService.userSignup(userCreateDto)).rejects.toThrow(AppError.badRequest(
...
describe("User signin", ()=> {
    ...
    const refreshToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IlRoaXMgaXMgcmVmcmVzaCB0b2tlbiIsImlhdCI6MTUxNjIzOTAyMn0.tPMjMMScVLLdksdC4qDZ7uzuapfuxaj2WZnhHTgPyYs'
    afterEach(()=> {
        jest.clearAllMocks()
    });
    ...
    it("should be able to sigh in", async()=> {
        ...
        (
            compare as jest.MockedFunction<typeof compare>
        ...
        const payloadToCompare = userLoginResponse(userData, accessToken, refreshToken);
        expect(getExistingUser).toHaveBeenCalled();
        expect(compare).toHaveBeenCalledWith(userData.password, userSignInDto.password);
    ...
    it("should return bad request when existing user not found", async()=> {
        ...
        await expect(()=> UserService.userSignin(userSignInDto)).rejects.toThrow(AppError.badRequest(
