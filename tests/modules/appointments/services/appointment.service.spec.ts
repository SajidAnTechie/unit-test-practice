...
describe("Create Appointment", ()=> {
    ...
    it("should return created appointment json body", async()=> {
        ...
        //act
        const appointment = await AppointmentService.createAppointment(createAppointmentDto);
        //assert
    ...
    it("should return bad request when appointment is null", async()=> {
        ...
        await expect(()=> AppointmentService.createAppointment(createAppointmentDto)).rejects.toThrow();
        await expect(()=> AppointmentService.createAppointment(createAppointmentDto)).rejects.toThrow(AppError.badRequest(`Error while creating the appointment.`))
    });
});
...
describe("Get appointments by user id", ()=> {
    ...
    it("should return appointments with given valid user id", async()=> {
        (
            getAppointmentsByUserId as jest.MockedFunction<typeof getAppointmentsByUserId>
        ).mockResolvedValue(Promise.resolve(payload));
        
        const appointments = await AppointmentService.getUserCreatedAppointments({ userId });
        expect(getAppointmentsByUserId).toHaveBeenCalled();
        expect(getAppointmentsByUserId).toHaveBeenCalledWith(userId, 10, 1, "id", "asc");
    ...
    it("should return bad request when appointment is null", async()=> {
        ...
        await expect(()=> AppointmentService.getUserCreatedAppointments({ userId })).rejects.toThrow();
        await expect(()=> AppointmentService.getUserCreatedAppointments({ userId })).rejects.toThrow(AppError.badRequest(`Error while fetching the appointments`));
    });
});
...
describe("Get appointment by appointment id", ()=> {
    ...
    it("should return an appointment with given valid appointment id", async()=> {
        ...
        const appointment = await AppointmentService.getAppointment(appointmentId, userId);
    ...
    it("should return bad request when appointment is null", async()=> {
        ...
        await expect(()=> AppointmentService.getAppointment(appointmentId, userId)).rejects.toThrow();
        await expect(()=> AppointmentService.getAppointment(appointmentId, userId)).rejects.toThrow(AppError.notFound(
...
describe("Update appointments by appointment id", ()=> {
    ...
    it("should return updated appointments counts with given valid appointment id", async()=> {
        ...
        const msg = await AppointmentService.updateAppointment(appointmentId, payload);
        expect(getAppointmentById).toHaveBeenCalled();
    ...
    it("should return bad request when appointment not found", async()=> {
        ...
        await expect(()=> AppointmentService.updateAppointment(appointmentId, payload)).rejects.toThrow();
        await expect(()=> AppointmentService.updateAppointment(appointmentId, payload)).rejects.toThrow(AppError.notFound(
            `Appointment with Id ${appointmentId} could not be found.`
          ));
    });
    ...
    it("should return bad request when unable to update an appointment", async()=> {
        ...
        await expect(()=> AppointmentService.updateAppointment(appointmentId, payload)).rejects.toThrow();
        await expect(()=> AppointmentService.updateAppointment(appointmentId, payload)).rejects.toThrow(AppError.badRequest(
...
describe("Update appointment", ()=> {
    ...
    it("should return deleted appointments counts with given valid appointment and user id", async()=> {
        ...
        const msg = await AppointmentService.deleteAppointment(appointmentId, userId);
    ...
    it("should return bad request when appointment not found", async()=> {
        ...
        await expect(()=> AppointmentService.deleteAppointment(appointmentId, userId)).rejects.toThrow();
        await expect(()=> AppointmentService.deleteAppointment(appointmentId, userId)).rejects.toThrow(AppError.notFound(
